...
if(isset($_GET['add_student'])){
	...
	<tbody>
		<?php 
		$students = get_data_awards(null,$_GET['id_student']);
		$ttotal_point = 0;
		foreach($students as $stu){
			$total_point = $stu['total_hour'] * $stu['point'];
			$ttotal_point += $total_point;
			...
			echo "<tr> 
						...
						<td>".$stu['role']."</td>
						<td>".$stu['point']."</td>
						<td>".$stu['total_hour']."</td>
						<td>".$total_point."</td>
				</tr>";
		}
		?>
	<tr><td colspan="5" align="right">Total : </td><td><?php echo $ttotal_point ?></td></tr>
	</tbody>
	</table>
		...
		foreach($students as $stu){
			...
			echo "<tr style='background-color:#000'> <td>".$stu['name']."</td>
						...
						<td>
							...
							<a title='List Activity' href='?activity=true&id_student=".$stu['id']."'><span class='glyphicon glyphicon-th-list'></span> </a>
